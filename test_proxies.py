...
Date: 21/03/2021
Author: AlexxLy
Description: lit un fichier contenant une liste de proxys et détermine si cette liste est bonne ou non.
             Chaque ligne du fichier doit être au format ip: port
'''
...
class TestProxies:
    ...
    def display(self):
        ...
        print(f'Compler: {complete}%')
        print(f'Active la page: {self.active_brs}')
        print(f'Proxies en marche: {worked_per}% [{worked}]')
        print(f'Proxies erreur: {failed_per}% [{failed}]')
    ...
    def examine(self):
        ...
        if worked == 0:
            print('mauvaise liste de proxy')
        elif (failed - worked) >= 0.1:
            print('mauvaise liste de proxy')
        elif (failed - worked) == 0:
            print('mauvaise liste de proxy')
        else:
            print('Bonne liste de proxy')
